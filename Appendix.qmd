...
```{r}
#| label: fig-Fig_S33
#| fig-cap: "Bias on effect estimates for different ML algorithms in two different simulated causal simulations (a and b). Sample sizes are so large that stochastic effects can be excluded. Effects of the ML models were inferred using average conditional effects. Row a) shows results for simulations with with features (Pearson correlation factor = 0.5) with effect sizes (X~1~: 1.0, X~2~: -0.5, X~3~: 1.0). Row b) shows the results for simulations with X~1~ and X~2~ being strongly correlated (Pearson correlation factor = 0.99) but only X~1~ has an effect on y. "
#| fig-width: 12
#| fig-height: 7
#| warning: false
#| message: false
...
sc = c("collinearity_0.99", "confounder_unequal")
...
sc = c("effects", "collinearity_0.90", "collinearity_0.99", "confounder_unequal")

#cairo_pdf("plots/Fig_2.pdf", width = 12, height = 7)

par(mfcol = c(4,4), mar = c(5,0.5, 2, 1.4), oma = c(1, 2, 2, 1))
labs =  c("LM","RF",  "BRT", "NN","Dropout", "l1", "l2", "Elastic-net")
cex_fac = 1.3
minArrow = function(x) sapply(x, function(xx) max(c(0.1, xx)))
...
```{r}
res0 = readRDS("results/NN_learning_0.RDS")
res50 = readRDS("results/NN_learning_50.RDS")
res500 = readRDS("results/NN_learning_500.RDS")

a = 
  ggplot(data = res50, aes(X1, X2)) +
...
grid.arrange(a, b, c, nrow = 2, ncol =2, layout_matrix = matrix(c(1, 2, 3, 3), 2, 2, byrow = TRUE))
```
