  ...
  result_list = 
    parallel::parLapplyLB(cl, 1:samples, function(i) {
      ...
      m = ranger(Y ~.,
                 ...
                 num.trees = 100L,
                 num.threads = 3L)
      eff = (marginalEffects(m, data = data.frame(train), interactions=TRUE, max_indices = 2)$mean)
      ...
      eff = (marginalEffects(m, data = data.frame(train)[,-1], interactions=TRUE, max_indices = 2)$mean)
      eff = c(diag(eff), eff[1, 2])
      ...
      try({
        ...
        eff = (marginalEffects(m, interactions = TRUE, max_indices = 2, device = device)$mean)
        eff = c(diag(eff), eff[1, 2])
      ...
      ## Elastic-net tuned
      m = glmnet(model.matrix(~.^2, data = data.frame(train[,-1])), train[,1], alpha = EN_alpha, lambda = EN_lambda)
      eff = (marginalEffects(m, data = train[,-1],alpha = EN_alpha, lambda = EN_lambda, interactions = TRUE, max_indices = 2,formula = function(d) model.matrix(~.^2, data = data.frame(d)))$mean)
      eff = c(diag(eff), eff[1, 2])
      pred = predict(m, newx = model.matrix(~.^2, data = data.frame(test[,-1])))
...
saveRDS(results, "results/data_poor_small_MSE_interactions.RDS")
...
saveRDS(results, "results/data_poor_mid_MSE_interactions.RDS")
...
results = get_result(sim)
saveRDS(results, "results/data_poor_big_MSE_interactions.RDS")
