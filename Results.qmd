...
extract_results = function(path, N = "low", tuned = "pred") {
  ...
  colnames(bias_low) = c("bias_1_Main", "bias_0_Main") #, "bias_1_Inter", "bias_0_Inter")
  colnames(var_low) = c("var_1_Main", "var_0_Main") #, "var_1_Inter", "var_0_Inter")
  rownames(bias_low) = rownames(var_low) = c( "LM","RF", "BRT", "NN","glmnet")
  
  bias_low_L = bias_low %>% data.frame %>% mutate(method = rownames(.)) %>%  pivot_longer(cols = c("bias_1_Main", "bias_0_Main"), values_to = "bias") %>% mutate(N = N, tuned = tuned)
  var_low_L = var_low %>% data.frame %>% mutate(method = rownames(.)) %>%  pivot_longer(cols = c("var_1_Main", "var_0_Main"), values_to = "var") %>% mutate(N = N, tuned = tuned)
  
  data_bias = bias_low_L
  data_var = var_low_L
...
res_pred = do.call(rbind, lapply(c("small"), function(n) extract_results(paste0("results/data_poor_",n ,"_MSE_50.RDS"), n, "pred")))
res_eff= do.call(rbind, lapply(c("small"), function(n) extract_results(paste0("results/data_poor_",n ,"_MSE_50.RDS"), n, "pred")))
...
methods = c("LM", "RF", "BRT", "NN", "glmnet")
sizes = c("small", "mid", "big")
y_labels = c("N = 100", "N = 600", "N = 2000")
for(j in 1:1) {
  for(i in 1:5) {
    if(i == 1) axes = TRUE
    else axes = FALSE
    if(j ==1) create_gapped_stacked(tmp = rbind(res_pred[res_pred$method == methods[i] & res_pred$N== sizes[j], ],
                                                res_eff[res_pred$method == methods[i] & res_eff$N== sizes[j], ]), 
                                    upper = 1.0,
                                    axes = axes, to = c(0, 1.0), labels1 = c("0.0", "1.0", "2.0", "3.0", "4.0"), gapped = FALSE)
    else create_gapped_stacked(tmp = rbind(res_pred[res_pred$method == methods[i] & res_pred$N== sizes[j], ],
                                                res_eff[res_pred$method == methods[i] & res_eff$N== sizes[j], ]), 
                               upper = 1.0,
                               to = c(0, 1.0),
                               axes = axes,
                                labels1 = c("0.0", "0.2", "0.4", "0.6", "0.8"), gapped = FALSE)
    if(j == 3){
...
for(j in 1:3) {
  for(i in 1:5) {
    ...
    tmp_data = rbind(res_pred[res_pred$method == methods[i] & res_pred$N== sizes[j], ],
                                                res_eff[res_pred$method == methods[i] & res_eff$N== sizes[j], ])
    
    
    
    create_gapped_stacked(tmp = rbind(res_pred[res_pred$method == methods[i] & res_pred$N== sizes[j], ],
                                                res_eff[res_pred$method == methods[i] & res_eff$N== sizes[j], ]), 
                                    upper = 1.0,
                                    axes = axes, to = c(0, 1.0), labels1 = c("0.0", "1.0", "2.0", "3.0", "4.0"), gapped = FALSE)
  }
...
sapply(1:3, function(k) {
  ...
  methods = c("NN", "BRT", "RF", "Elastic_net")
  results = lapply(1:4, function(i) {
    tmp = get_coefficients(i, gam=FALSE)$hyper$pred %>% select(-var_effect, -bias_zero, -var_zero, -bias_pred, -var_pred, -mse_eff, -mse_zero, -bias_effect, -mse_pred)
    names(tmp) = paste0(c("NN", "BRT", "RF", "EN")[i], "_", names(tmp))
    return(tmp)
    })
