...
get_coefficients = function(i) {
  
  out = list()
  
  method = methods[i]
  tmp = paths[i]
  raw = do.call(rbind, readRDS(tmp) )
  raw =  raw[abs(raw$mse_1) < 5.98,]
  rwa =  raw[abs(raw$mse_2) < 5.98,]
  raw = raw[complete.cases(raw),]
  tmp = raw
  
  if(method == "NN") tmp = tmp %>% select(-dropout)
  
  if(method != "NN" ) {tmp[,-(ncol(tmp):(ncol(tmp)-3))] = scale( tmp[,-(ncol(tmp):(ncol(tmp)-3))] )
  } else { tmp[,-c(1, ncol(tmp):(ncol(tmp)-3))] = scale( tmp[,-c(1, ncol(tmp):(ncol(tmp)-3))] ) }
  tmp$mse_1 = tmp$mse_1 - mean(tmp$mse_1, na.rm=TRUE)
  tmp$mse_pred = tmp$mse_pred - mean(tmp$mse_pred, na.rm = TRUE)
  tmp$mse_0 = tmp$mse_0 - mean(tmp$mse_0, na.rm=TRUE)
  lm_eff = lm( mse_1 ~ 0 + ., data = tmp %>% select( -mse_0, -mse_2, -mse_pred ))
  lm_pred = lm( mse_pred ~0+ ., data = tmp %>% select( -mse_0, -mse_2, -mse_1))
  lm_zero = lm( mse_0 ~0+ ., data = tmp %>% select( -mse_pred, -mse_2, -mse_1))


  out$lm = list(eff = lm_eff, pred = lm_pred)
  out$anova = list(eff = summary(aov(lm_eff)), pred = summary(aov(lm_pred)))
  
  
  coefs_eff = summary( lm_eff )$coefficients
  coefs_pred = summary( lm_pred )$coefficients
  coefs_zero = summary( lm_zero )$coefficients

  
  if(method == "NN") {
    
    tmp$depth = tmp$depth + rnorm(nrow(tmp), 0, 0.001)
    cols = tmp %>% select( -mse_0, -mse_2, -mse_pred ) %>% colnames()
    form1 = paste0("mse_1 ~ 0 +activations +" , paste0("s(", cols[-length(cols)][-1], ")", collapse="+"))
    cols = tmp %>% select( -mse_1, -mse_2, -mse_pred ) %>% colnames()
    form2 = paste0("mse_0 ~ 0 +activations +" , paste0("s(", cols[-length(cols)][-1], ")", collapse="+"))
    cols = tmp %>% select( -mse_1, -mse_2, -mse_0 ) %>% colnames()
    form3 = paste0("mse_pred ~ 0 +activations +" , paste0("s(", cols[-length(cols)][-1], ")", collapse="+"))

  } else {
    cols = tmp %>% select( -mse_0, -mse_2, -mse_pred ) %>% colnames()
    form1 = paste0("mse_1 ~ 0  +" , paste0("s(", cols[-length(cols)], ")", collapse="+"))
    cols = tmp %>% select( -mse_1, -mse_2, -mse_pred ) %>% colnames()
    form2 = paste0("mse_0 ~ 0 + " , paste0("s(", cols[-length(cols)], ")", collapse="+"))
    cols = tmp %>% select( -mse_1, -mse_2, -mse_0 ) %>% colnames()
    form3 = paste0("mse_pred ~ 0 + " , paste0("s(", cols[-length(cols)], ")", collapse="+"))
  }
  
  out$gam_eff = mgcv::gam(as.formula(form1), data = tmp)
  out$gam_zero = mgcv::gam(as.formula(form2), data = tmp)
  out$gam_pred = mgcv::gam(as.formula(form3), data = tmp)
  ...
  # brt_eff= xgboost::xgboost(data=xgboost::xgb.DMatrix(as.matrix(tmp %>% select( -mse_0, -mse_2, -mse_pred, -mse_1) ),
  #                                                        label = as.matrix(tmp %>% select(mse_1))),
  #                              nrounds = 300,
  #                              objective="reg:squarederror", nthread = 1, verbose = 0)
  # 
  # brt_zero= xgboost::xgboost(data=xgboost::xgb.DMatrix(as.matrix(tmp %>% select( -mse_0, -mse_2, -mse_pred, -mse_1) ),
  #                                                        label = as.matrix(tmp %>% select(mse_0))),
  #                              nrounds = 300,
  #                              objective="reg:squarederror", nthread = 1, verbose = 0)
  # 
  # brt_pred= xgboost::xgboost(data=xgboost::xgb.DMatrix(as.matrix(tmp %>% select( -mse_0, -mse_2, -mse_pred, -mse_1) ),
  #                                                        label = as.matrix(tmp %>% select(mse_pred))),
  #                              nrounds = 300,
  #                              objective="reg:squarederror", nthread = 1, verbose = 0)
  rf_eff = ranger::ranger(mse_1~., data = as.matrix(tmp %>% select( -mse_0, -mse_2, -mse_pred)), num.trees = 2000, importance = "permutation")
  rf_pred = ranger::ranger(mse_pred~., data = as.matrix(tmp %>% select( -mse_0, -mse_2, -mse_1)), num.trees = 2000, importance = "permutation")
  rf_zero = ranger::ranger(mse_0~., data = as.matrix(tmp %>% select( -mse_pred, -mse_2, -mse_1)), num.trees = 2000, importance = "permutation")

  pred_eff = which.min(predict(rf_eff, data = as.matrix(tmp %>% select( -mse_0, -mse_2, -mse_pred)))$predictions)
  pred_pred = which.min(predict(rf_pred, data = as.matrix(tmp %>% select( -mse_0, -mse_2, -mse_pred)))$predictions)

  
  # pred_eff = which.min(predict(brt_eff, newdata = xgboost::xgb.DMatrix( as.matrix(tmp %>% select( -mse_0, -mse_2, -mse_pred, -mse_1) ) )))
  # pred_pred = which.min(predict(brt_pred, newdata = xgboost::xgb.DMatrix( as.matrix(tmp %>% select( -mse_0, -mse_2, -mse_pred, -mse_1) ) )))
  # 
  out$tmp = tmp
  ...
  out$brts = list(eff = rf_eff, pred = rf_pred)
  out$preds = list(eff = pred_eff, pred = pred_pred)
  # out$importances = list(eff =   as.data.frame(xgboost::xgb.importance(model = brt_eff)), 
  #                        pred =   as.data.frame(xgboost::xgb.importance(model = brt_pred)),
  #                        zero = as.data.frame(xgboost::xgb.importance(model = brt_zero)))
  out$importances = list(eff =   transf_imp(importance(rf_eff)),
                         pred =   transf_imp(importance(rf_pred)),
                         zero = transf_imp(importance(rf_zero)))
  out$hyper = list(eff = raw[pred_eff,], pred = raw[pred_pred,])
...
get_coefficients = function(i) {
  ...
  tmp = paths[i]
  raw = readRDS(tmp)
  parameter = raw[[1]][,-(ncol(raw[[1]]):(ncol(raw[[1]])-2))]
  ...
  if(method != "NN" ) {tmp[,-(ncol(tmp):(ncol(tmp)-6))] = scale( tmp[,-(ncol(tmp):(ncol(tmp)-6))] )
  } else { tmp[,-c(1, ncol(tmp):(ncol(tmp)-6))] = scale( tmp[,-c(1, ncol(tmp):(ncol(tmp)-6))] ) }
  
  tmp[,(ncol(tmp):(ncol(tmp)-6))] = sapply(tmp[,(ncol(tmp):(ncol(tmp)-6))], function(df) df - mean(df))
  
  out$lm_eff = lm(  mse_eff ~ 0 + .,data = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(mse_eff )))
  out$lm_pred = lm( mse_pred ~0+ ., data = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(mse_pred )))
  out$lm_zero = lm( mse_zero ~0+ ., data = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(mse_zero )))
  out$lm_bias_eff = lm(  bias_effect ~ 0 + .,data = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(bias_effect )))
  out$lm_var_eff = lm( var_effect ~0+ ., data = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(var_effect )))
  out$lm_bias_zero = lm(  bias_zero ~ 0 + .,data = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(bias_zero )))
  out$lm_var_zero = lm( var_zero ~0+ ., data = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(var_zero )))
  ...
  if(method == "NN") {
    ...
    tmp$depth = tmp$depth + rnorm(nrow(tmp), 0, 0.001)
    cols = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(mse_eff )) %>% colnames()
    form1 = paste0("mse_eff ~ 0 +activations +" , paste0("s(", cols[-length(cols)][-1], ")", collapse="+"))
    cols = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(mse_zero )) %>% colnames()
    form2 = paste0("mse_zero ~ 0 +activations +" , paste0("s(", cols[-length(cols)][-1], ")", collapse="+"))
    cols = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(mse_pred )) %>% colnames()
    form3 = paste0("mse_pred ~ 0 +activations +" , paste0("s(", cols[-length(cols)][-1], ")", collapse="+"))
    
    cols = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(bias_effect )) %>% colnames()
    form4 = paste0("bias_effect ~ 0 +activations +" , paste0("s(", cols[-length(cols)][-1], ")", collapse="+"))
    cols = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(var_effect )) %>% colnames()
    form5 = paste0("var_effect ~ 0 +activations +" , paste0("s(", cols[-length(cols)][-1], ")", collapse="+"))
    
    cols = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(bias_zero )) %>% colnames()
    form6 = paste0("bias_zero ~ 0 +activations +" , paste0("s(", cols[-length(cols)][-1], ")", collapse="+"))
    cols = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(var_zero )) %>% colnames()
  ...
  rf_eff = ranger::ranger(  mse_eff ~ 0 + .,data = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(mse_eff )), num.trees= 2000, importance = "permutation" ) 
  rf_pred = ranger::ranger( mse_pred ~0+ ., data = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(mse_pred )), num.trees= 2000, importance = "permutation" ) 
  rf_zero = ranger::ranger( mse_zero ~0+ ., data = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(mse_zero )), num.trees= 2000, importance = "permutation" ) 
  rf_bias_effect = ranger::ranger(  bias_effect ~ 0 + .,data = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(bias_effect )), num.trees= 2000, importance = "permutation" ) 
  rf_var_effect = ranger::ranger( var_effect ~0+ ., data = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(var_effect )), num.trees= 2000, importance = "permutation" ) 
  rf_bias_zero = ranger::ranger(  bias_zero ~ 0 + .,data = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(bias_zero )), num.trees= 2000, importance = "permutation" ) 
  rf_var_zero = ranger::ranger( var_zero ~0+ ., data = cbind( tmp[,-(ncol(tmp):(ncol(tmp)-6))] ,tmp %>% select(var_zero )), num.trees= 2000, importance = "permutation" ) 
  ...
  out$importances = list(eff =   transf_imp(importance(rf_eff)),
                         pred =   transf_imp(importance(rf_pred)),
                         zero = transf_imp(importance(rf_zero)),
                         bias_effect = transf_imp(importance(rf_bias_effect)),
                         var_effect = transf_imp(importance(rf_var_effect)),
                         bias_zerp = transf_imp(importance(rf_bias_zero)),
                         var_zerp = transf_imp(importance(rf_bias_zero))                         
                         )
  ...
  out$data =   rbind(
    ...
    as.data.frame(coefs_bias_effect[,c(1, 2, 4)]) %>% mutate(group = "bias_zero") %>% mutate(hyper = rownames(coefs_pred), algorithm = method),
    as.data.frame(coefs_var_effect[,c(1, 2, 4)]) %>% mutate(group = "var_zero") %>% mutate(hyper = rownames(coefs_pred), algorithm = method)
  )
...
sapply(1:3, function(k) {
  ...
  paths <<- c(
    paste0("results/NN_pars_",tmp[1],"_",tmp[2],".RDS"),
    paste0("results/BRT_pars_",tmp[1],"_",tmp[2],".RDS"),
    paste0("results/RF_pars_",tmp[1],"_",tmp[2],".RDS"),
    paste0("results/Elastic_net_pars_",tmp[1],"_",tmp[2],".RDS")
  )
  methods = c("NN", "BRT", "RF", "Elastic_net")
  results = lapply(1:4, function(i) {
    tmp = get_coefficients(i)$hyper$eff %>% select(-mse_1, -mse_2, -mse_0, -mse_pred)
...
sapply(1:3, function(k) {
  ...
  methods = c("NN", "BRT", "RF", "Elastic_net")
  results = lapply(1:4, function(i) {
    tmp = get_coefficients(i)$hyper$pred %>% select(-mse_1, -mse_2, -mse_0, -mse_pred)
    names(tmp) = paste0(c("NN", "BRT", "RF", "EN")[i], "_", names(tmp))
    return(tmp)
    })
