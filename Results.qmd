...
g = 
  ggplot(data %>% filter(!method %in% c("NN_Drop", "L1", "L2")), aes(fill=name, y=value, x=which_bias)) + 
    ...
    facet_grid(N~ method, 
               labeller = labeller(N = c("low" = "N = 300", "med" = "N = 500", "large" = "N = 5,000")),
               scales="free") +
    geom_text(aes(x=2.5, y = median(c(min(value, na.rm = TRUE), max(value, na.rm = TRUE)) ), label = label), size = 3)+
    theme_bw()  + 
    scale_fill_manual("",  values = c("bias" = "#96c6ed", "var" = "#e0acd5" ) )+
    scale_y_continuous(breaks = c(0.25, 0.0, -0.25, -0.5, -0.75, -1.0), limits = c(-1.0, 0.25)) +
    labs(x = "", y = "") +
    theme_bw() +
